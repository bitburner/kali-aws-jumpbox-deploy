...
ssh -o "StrictHostKeyChecking no" -t -i "$ssh_key" kali@$public_dns 'bash -s' < commands.sh; scp -i "$ssh_key" interact.sh kali@$public_dns:~/; ssh -o "StrictHostKeyChecking no" -t -i "$ssh_key" kali@$public_dns 'bash interact.sh'
